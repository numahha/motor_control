...
#define INPUT_MAGNITUDE  (2.5) // voltage
#define PERIOD_US                (    2000*1000 ) // micro second

#define FILE_NAME_DATA      "./freq_responce.dat"
...
int main()
{
  ...
  for(i=0;i<TOTAL_DATA_LENGTH;i++){
    ...
    if(i >= ZERO_DATA_LENGTH){
      da_v = INPUT_MAGNITUDE * sin(  (i-ZERO_DATA_LENGTH)*(2.*M_PI/PERIOD_LENGTH)  );
